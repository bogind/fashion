...
L.Control.FuseSearch = L.Control.extend({
    ...
    createResultItem: function(props, container, popup,map) {
        ...
        if (undefined !== popup) {
            ...
            resultItem.onclick = function() {
                ...
                if (window.matchMedia("(max-width:480px)").matches) {
                    ...
                    res = feature.layer
                    //console.log(f)
                    // Dror: Had to change this since we don't have any popups, panning is useless and can't use the map object here.
                    return([res])
                    //f._map.setZoomAround(f.getCenter(),14)
                   // _this._panAndPopup(feature, popup);
                }
            };
        }
